...
Contracting Works use a REST API, documented at [OpenAPI 3.0](https://contracting-extest-clientapi.azurewebsites.net/swagger/v1/swagger.json) and is available for interactive testing through a [Swagger page.](https://contracting-extest-clientapi.azurewebsites.net/swagger/index.html)
...
In addition to the mandatory clientId (part of the request URL), all methods support "***noEvents***" (boolean query parameter): If set to true, the upsert operation will not cause data change events on the event bus. This functionality is most useful when migrating data, but also useful for avoiding integration event loops.
